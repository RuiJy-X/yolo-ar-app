# backend.spec  (at yolo-ar-app/ root)
import os
...
a = Analysis(
    [os.path.join(BACKEND_SRC, 'websocket_api.py')],
    pathex=[BACKEND_SRC],
    binaries=[],
    datas=[
        # YOLO model files
        (os.path.join(BACKEND_SRC, 'yolo-best.pt'),    '.'),
        (os.path.join(BACKEND_SRC, 'yolo11n-pose.pt'), '.'),
        (os.path.join(BACKEND_SRC, 'yolo26n-pose.pt'), '.'),

        # Action model checkpoints
        (os.path.join(BACKEND_SRC, 'results', 'Frame_16.pt'), 'results'),
        (os.path.join(BACKEND_SRC, 'results', 'Frame_32.pt'), 'results'),
        (os.path.join(BACKEND_SRC, 'results', 'Frame_64.pt'), 'results'),
        ...
        # FFmpeg binaries
        (os.path.join(BACKEND_SRC, 'ffmpeg', 'ffmpeg.exe'),  'ffmpeg'),
        (os.path.join(BACKEND_SRC, 'ffmpeg', 'ffplay.exe'),  'ffmpeg'),
        (os.path.join(BACKEND_SRC, 'ffmpeg', 'ffprobe.exe'), 'ffmpeg'),
    ...
    'torch._dynamo.backends.tensorrt',
    ...
    'torch._dynamo.create_parameter_op',
    ...
    'torch._export.db.examples.unsupported_operator',
    ...
    'torch._export.passes.constant_folding',
    ...
    'torch._functorch.top_operators_github_usage',
    ...
    'torch._inductor.autoheuristic.artifacts._MMRankingA100',
    'torch._inductor.autoheuristic.artifacts._MMRankingH100',
    'torch._inductor.autoheuristic.artifacts._MixedMMA100',
    'torch._inductor.autoheuristic.artifacts._MixedMMH100',
    'torch._inductor.autoheuristic.artifacts._PadMMA100',
    ...
    'torch._inductor.codegen.codegen_device_driver',
    ...
    'torch._inductor.fx_passes.serialized_patterns._sfdp_pattern_18',
    'torch._inductor.fx_passes.serialized_patterns._sfdp_pattern_19',
    ...
    'torch._inductor.fx_passes.serialized_patterns.addmm_pattern',
    'torch._inductor.fx_passes.serialized_patterns.bmm_pattern',
    'torch._inductor.fx_passes.serialized_patterns.mm_pattern',
    ...
    'torch._inductor.package.build_package',
    ...
    'torch.cuda.tunable',
    'torch.distributed',
    'torch.distributed._checkpointable',
    ...
    'torch.distributed._shard.checkpoint._checkpointer',
    ...
    'torch.distributed._shard.checkpoint._version',
    ...
    'torch.distributed._shard.checkpoint.logger',
    'torch.distributed._shard.checkpoint.logging_handlers',
    ...
    'torch.distributed._shard.checkpoint.staging',
    ...
    'torch.distributed.tensor._shards_wrapper',
    ...
    'torch.distributed.tensor.debug._op_coverage',
    ...
    'torch.distributed.tensor.experimental._tp_transform',
    ...
    'torch.quantization._numeric_suite_fx',
    ...
    'torch.quantization.fx._equalize',
    ...
    'torch.testing._internal.custom_tensor',
    ...
    'torch.utils._pytree',
    'torch.utils._stats',
    'torch.utils._strobelight',
    'torch.utils._strobelight.cli_function_profiler',
    'torch.utils._sympy',
    ...
    runtime_hooks=[],
    excludes=[
        'notebook', 'IPython', 'jedi', 'tensorflow',
    'torchaudio',        # you don't use audio
    'torchtext', 
        # tensorflow is in your _internal but likely unused — test removing it:
        # 'tensorflow',
    ],
    cipher=block_cipher,
    noarchive=False,
...
exe = EXE(
    ...
    name='yolo-ar-backend',   # matches your existing backend-dist output name
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    console=False,
    icon=os.path.join('public', 'favicon.ico'),  # swap for .ico if you have one
)
...
coll = COLLECT(
    ...
    upx=True,
    upx_exclude=[],
    name='yolo-ar-backend',   # output folder: backend-dist/yolo-ar-backend/
)